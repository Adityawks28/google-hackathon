...
function UploadContent() {
  ...
  const [miniLessonConcepts, setMiniLessonConcepts] = useState<MiniLessonConcept[]>([]);
  ...
  function addMiniLessonConcept() {
    setMiniLessonConcepts([...miniLessonConcepts, { title: "", explanation: "", codeExample: "", symbolBreakdown: [] }]);
  }

  function updateMiniLessonConcept(index: number, field: "title" | "explanation" | "codeExample", value: string) {
  ...
  function addSymbolBreakdown(conceptIndex: number) {
    ...
    updated[conceptIndex] = { ...updated[conceptIndex], symbolBreakdown: [...symbols, { symbol: "", meaning: "" }] };
    setMiniLessonConcepts(updated);
  }

  function updateSymbolBreakdown(conceptIndex: number, symbolIndex: number, field: keyof SymbolBreakdown, value: string) {
    const updated = [...miniLessonConcepts];
    const symbols = [...(updated[conceptIndex].symbolBreakdown || [])];
    symbols[symbolIndex] = { ...symbols[symbolIndex], [field]: value };
    updated[conceptIndex] = { ...updated[conceptIndex], symbolBreakdown: symbols };
    setMiniLessonConcepts(updated);
  }

  function removeSymbolBreakdown(conceptIndex: number, symbolIndex: number) {
    const updated = [...miniLessonConcepts];
    const symbols = (updated[conceptIndex].symbolBreakdown || []).filter((_, i) => i !== symbolIndex);
    updated[conceptIndex] = { ...updated[conceptIndex], symbolBreakdown: symbols };
    setMiniLessonConcepts(updated);
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">
              ...
              <span className="text-gray-400 font-normal">(optional — guided mode scaffold for beginners)</span>
            </label>
            <textarea
              value={beginnerStarterCode}
              onChange={(e) => setBeginnerStarterCode(e.target.value)}
              rows={6}
              placeholder={"Provide step-by-step scaffolded code with TODO comments and syntax hints for absolute beginners"}
              className="w-full rounded-md border border-gray-300 px-3 py-2 font-mono text-sm focus:border-blue-500 focus:outline-none"
            />
          </div>
          ...
          <div>
            <label className="mb-2 block text-sm font-medium text-gray-700">
              ...
              <span className="text-gray-400 font-normal">(optional — teach beginners the syntax they need)</span>
            </label>
            <div className="space-y-4">
              {miniLessonConcepts.map((concept, i) => (
                <div key={i} className="rounded-md border border-gray-200 bg-gray-50 p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-gray-500">Concept {i + 1}</span>
                    <button
                      type="button"
                  ...
                  <input
                    type="text"
                    value={concept.title}
                    onChange={(e) => updateMiniLessonConcept(i, "title", e.target.value)}
                    placeholder='Title (e.g., "What is a for loop?")'
                    className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none"
                  />
                  <textarea
                    value={concept.explanation}
                    onChange={(e) => updateMiniLessonConcept(i, "explanation", e.target.value)}
                  ...
                  <textarea
                    ...
                    onChange={(e) => updateMiniLessonConcept(i, "codeExample", e.target.value)}
                  ...
                  <div className="pt-2 border-t border-gray-200">
                    <span className="text-xs font-medium text-gray-500">Symbol Breakdowns (optional)</span>
                      ...
                      {(concept.symbolBreakdown || []).map((sb, si) => (
                        <div key={si} className="flex items-center gap-2">
                          <input
                            type="text"
                            value={sb.symbol}
                            onChange={(e) => updateSymbolBreakdown(i, si, "symbol", e.target.value)}
                          ...
                          <input
                            type="text"
                            value={sb.meaning}
                            onChange={(e) => updateSymbolBreakdown(i, si, "meaning", e.target.value)}
